    ...
    <div className='relative flex items-center w-full gap-3 p-3 border rounded-md border-primary'>
          ...
          {player_1 &&
            <div>
              <button onClick={()=>onRemovePlayer(player_1)}>
                <RemoveIcon className='text-errorRed'/>{player_1.full_name}
              </button>
            </div>
          }
          {player_2 &&
            <div>
              <button onClick={()=>onRemovePlayer(player_2)}>
                <RemoveIcon className='text-errorRed'/>{player_2.full_name}
              </button>
            </div>
          }
        </div>
      ...
      {!editOpen && !(player_1 && player_2) &&
        <div className='absolute bottom-2 right-2 bg-primary text-[12px] p-1 rounded-md flex items-center gap-1'>
            <CopyWrapper
                copyString={getInviteLinkTeam(uuid)}
                size={20}
            >
    ...
    <div className='flex items-center p-2 space-x-3'>
      ...
      <div className=''>
        ...
        <div className='text-[14px]'>
          <div>
            {player_1  && player_1.full_name}
          </div>
          <div>
            {player_2 && player_2.full_name}
          </div>
        </div>
      </div>
    </div>
...
const HomePre = ({events, img, is_owner, projected_start_date, projected_end_date, teams, user_team}) => {
  ...
  const onCreateTeamClick = async () => {
    const response = await fetchCreateSingleTeam(newTeamName, uuid, true)
    if (response.ok){
              ...
              >
                  { newTeamImg ?
                      ...
                      <div className='w-[100px] h-[100px] rounded-md flex items-center justify-center'>
                          <CameraAltIcon className='w-[100px] text-neutralLight' sx={{fontSize:60}}/>
                      </div>
                  }
              </label>
              {cropping &&
