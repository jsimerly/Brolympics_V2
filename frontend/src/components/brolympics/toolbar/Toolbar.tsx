...
const Toolbar = ({is_owner, default_team_uuid, default_event_uuid, default_event_type}) => {
    ...
    const PageButton = ({route, text, icon, path}) => (
        <div 
            ...
            className={`${pathRoute === path && 'text-primary'} flex flex-col items-center justify-center flex-1`}
        >
        {icon}
  ...
  return (
    <>
        ...
        <div className='fixed bottom-0 left-0 h-[60px] bg-neutral w-full border-t border-neutralLight flex justify-around items-center px-3'>
            <PageButton 
                route='home' text='Home' path='home'
            ...
            />
            <PageButton 
                route='standings' text='Standings'  path='standings'
                icon={<EmojiEventsOutlinedIcon sx={{fontSize:30}}/>}
            />
            {is_owner &&
                <PageButton 
                    route='manage' text='Manage'  path='manage'
